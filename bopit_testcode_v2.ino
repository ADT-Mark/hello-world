...
int readInput()
{
  ...
  //check each digital pin for user input
    if(digitalRead(greenButton) == HIGH && !doubleInput) //green button -> 0
    {
      if(inputPin != -1)
        inputPin = 0 ;
    ...
    if(digitalRead(redButton) == HIGH && !doubleInput) //red button -> 1
    {
      if(inputPin != -1)
        inputPin = 1 ;
      else //multiple inputs read
    ...
    if(digitalRead(yellowButton) == HIGH && !doubleInput) //yellow button -> 2
    {
      if(inputPin != -1)
        inputPin = 2 ;
      else //multiple inputs read
    ...
    if(digitalRead(blueButton) == HIGH && !doubleInput) //blue button -> 3
    {
      if(inputPin != -1)
        inputPin = 3 ;
      else //multiple inputs read
    ...
    if(digitalRead(joystickPress) == HIGH && !doubleInput) //joystick press ->4
    {
      if(inputPin != -1)
        inputPin = 4 ;
      else //multiple inputs read
    ...
    if(digitalRead(tiltLeft) == HIGH && !doubleInput) //tilt left -> 5
    {
      if(inputPin != -1)
        inputPin = 5 ;
      else //multiple inputs read
    ...
    if(digitalRead(tiltRight) == HIGH && !doubleInput) //tilt right -> 6
    {
      if(inputPin != -1)
        inputPin = 6 ;
      else //multiple inputs read
  ...
  //if the joystick is pushed halfway to the max value it will be considered as "active"
    if(analogRead(joystickX) < 256 && !doubleInput) //joystick left -> 7
    {
      if(inputPin != -1)
        inputPin = 8 ;
      else //multiple inputs read
    ...
    if(analogRead(joystickX) > 768 && !doubleInput) //joystick right -> 8
    {
      if(inputPin != -1)
        inputPin = 8 ;
      else //multiple inputs read
    ...
    if(analogRead(joystickY) < 256 && !doubleInput) //joystick down -> 9
    {
      if(inputPin != -1)
        inputPin = 9 ;
      else //multiple inputs read
    ...
    if(analogRead(joystickY) > 768 && !doubleInput) //joystick up -> 10
    {
      if(inputPin != -1)
        inputPin = 10 ;
      else //multiple inputs read
